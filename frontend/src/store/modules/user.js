...
async updateUserAdmin({ commit, dispatch }, { userId, data }) {
  commit('CLEAR_ERROR');
  try {
    // const response = await axios.put(`/accounts/users/${userId}/`, data); // 原来是 PUT
    const response = await axios.patch(`/accounts/users/${userId}/`, data); // 修改为 PATCH
    dispatch('fetchAllUsers');
    console.log('User updated by admin (PATCH)', response.data);
    return response.data;
  } catch (error) {
    const errorMessage = error.response?.data?.detail || JSON.stringify(error.response?.data) || 'Failed to update user'; // 更好地显示错误
    commit('SET_ERROR', errorMessage);
    throw new Error(errorMessage);
  }
},
